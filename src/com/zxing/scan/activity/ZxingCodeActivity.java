...
public class ZxingCodeActivity extends Activity implements Callback, View.OnClickListener {
	...
	private void getTheGoodsID(final String resultString, final Bitmap bitmap) {
		...
		//判断读出来的是不是一个http url链接
		if(StringUtils.isHttpUrl(resultString)){
			
			Map<String, String> params = StringUtils.parseHttpParams(resultString);
			if(params.containsKey("goods_id") && params.containsKey("store_id") && params.containsKey("goods_no")){
				
				//包含以上三个字段才调整，否则跳转到普通的webView界面
				Intent intent = new Intent(ZxingCodeActivity.this, ProductsDetailActivity.class);
				intent.putExtra(ProductsDetailActivity.BUNDLE_KEY_GOODS_ID, params.get("goods_id"));
				intent.putExtra(ProductsDetailActivity.USER_ID, AppContext.userId);
				intent.putExtra(ProductsDetailActivity.STORE_ID, params.get("store_id"));
				intent.putExtra(ProductsDetailActivity.GOODS_NO, params.get("goods_no"));
				startActivity(intent);
				finish();
				
			}else{
				
				//只是一个普通的请求,用系统浏览器打开界面
				Intent intent = new Intent(this, WebViewActivity.class);
				intent.putExtra(WebViewActivity.URL, resultString);
				startActivity(intent);
				finish();
		...
		CustomResponseHandler handler = new CustomResponseHandler(this, true) {
			...
			public void onRefreshData(String content) {
				...
				if (bean.type > 0) {
					Intent intent = new Intent(ZxingCodeActivity.this, ProductsDetailActivity.class);
					intent.putExtra(ProductsDetailActivity.BUNDLE_KEY_GOODS_ID, bean.commodityId);
