...
public class FlashPrefectureActivity extends BaseActivity implements OnRefreshListener2 {
	...
	private void loadPageData(int loadType) {
		...
		RequstClient.flashGoods(mPageindex, id, handler);
	}
	...
	private void updataView() {
		if (!StringUtils.isBlank(data.ACTIVE_BANNER)) {
			ImageManager.LoadWithServer(data.ACTIVE_BANNER, bannerImage);
		}
		if (!StringUtils.isBlank(data.ACTIVE_NAME)) {
			ab_title.setText(data.ACTIVE_NAME);
		}
		if (!StringUtils.isBlank(data.BEGINTIME) && !StringUtils.isBlank(data.ENDTIME)
				&& !StringUtils.isBlank(data.curTime)) {
			TextTask(data.BEGINTIME, data.ENDTIME, data.curTime);
		}
	}
