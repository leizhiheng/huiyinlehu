...
public class TheShowSearchActivity extends BaseActivity {
	...
	private void AttentionData(int spotlightUserId) {
		...
		if (AppContext.userId != null && !"".equals(AppContext.userId)) {

			if (AppContext.userId.equals(spotlightUserId + "")) {
				Toast.makeText(mContext, "不能关注自己！", Toast.LENGTH_SHORT).show();
			} else {

				RequstClient.appAttention(handler, AppContext.userId,
						spotlightUserId + "");
			}
		} else {
	...
	private void CancelAttentionData(int spotlightUserId) {
		...
		if (AppContext.userId != null && !"".equals(AppContext.userId)) {
			...
			RequstClient.appCancelAttention(handler, AppContext.userId,
					spotlightUserId + "");
		} else {
			Toast.makeText(mContext, "请登录！！！", Toast.LENGTH_SHORT).show();
		}
	...
	private void addLikeData(int spotlightId) {
		...
		if (AppContext.userId != null && !"".equals(AppContext.userId)) {
			...
			RequstClient.appShowLike(handler, AppContext.userId, spotlightId
					+ "");
		} else {
			Toast.makeText(mContext, "请登录！！！", Toast.LENGTH_SHORT).show();
		}
