...
public class ZhuanQuActivity extends BaseActivity implements OnClickListener,
	...
	private void initData() {
		...
		if (type == 2) {
			...
			requestData(id, type, pageIndex + "", true);
		} else if (type == 1) {
			// 分类聚合过来的
	...
	public void onItemClick(AdapterView<?> arg0, View arg1, int position,
		...
		intent.putExtra(ProductsDetailActivity.BUNDLE_KEY_GOODS_ID, bean
	...
	private void requestData(String id, final int type, final String page,
		...
		RequstClient.getZhuanQu(URLs.GET_ZHUANQU_DATE, id, page, type + "",
				new CustomResponseHandler(this, bool) {
	...
	public ZhuanquBean parseJson(String content, ZhuanquBean zBean) {
		try {
			...
			if (roots.getString("type").equals("1")) {
				JSONObject json = roots.optJSONObject("list");
				JSONArray goods = json.optJSONArray("pageBean");
				List<ZhuanquGoodbean> listGoodbeans = new ArrayList<ZhuanquGoodbean>();
				for (int i = 0; null != goods && i < goods.length(); i++) {
					JSONObject obj = goods.optJSONObject(i);
					ZhuanquGoodbean bean = new ZhuanquGoodbean();
					bean.setId(obj.optInt("ID"));
					bean.setImagePath(obj.optString("COMMODITY_IMAGE_PATH"));
					bean.setTitle(obj.optString("COMMODITY_NAME"));
					bean.setPrice(obj.optString("PRICE"));
					bean.setReprice(obj.optString("REFERENCE_PRICE"));
					bean.setSaleSum(obj.optString("SALES_VOLUME"));
					
					//好评度
					bean.setReviewPercent(obj.optString("REVIEW_PERCENT"));
					bean.setReviewNumber(obj.optString("REVIEW_NUMBER"));
					
					listGoodbeans.add(bean);
				}
				List<ZhuanquGallery> listGalleries = new ArrayList<ZhuanquGallery>();
				JSONArray gallerys = json.optJSONArray("prefecture");
				for (int i = 0; null != gallerys && i < gallerys.length(); i++) {
				...
				String layout = json.optString("LAYOUT");
				zBean.setTitle(title);
				zBean.setLayout(layout);
			} else {
				String errorMsg = roots.optString("msg");
...
//						URLs.IMAGE_URL + gallery.getImageUrl(), img);
                //图片获取统一处理用于处理省流量处理
                ImageManager.LoadWithServer(gallery.getImageUrl(),img);
	...
	protected void onDestroy() {
		...
		//释放资源
		ShareUtils.onActivityDestroyDo(this);
