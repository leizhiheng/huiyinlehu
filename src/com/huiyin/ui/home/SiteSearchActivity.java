...
public class SiteSearchActivity extends BaseActivity implements OnClickListener {
						...
						if (size <= 6) {
							if (size <= 3) {
								LinearLayout.LayoutParams params = new LayoutParams(
										LayoutParams.MATCH_PARENT, DensityUtil
										.dip2px(mContext, 125));
	...
	public void startSearch() {
		...
		if(!TextUtils.isEmpty(storeId)) {
			intent.putExtra(CategorySearchActivity.BUNDLE_STORE_ID, storeId);
		}
		intent.putExtra(CategorySearchActivity.BUNDLE_MARK, "1");
