...
//				.getSerializableExtra("selected_data");
        ...
        Object[] cobjs = (Object[]) getIntent().getSerializableExtra("selected_data");
        userChannelList = new ArrayList<ChannelItem>(cobjs.length);
        for (int i = 0; i < cobjs.length; i++) {
            ChannelItem user = (ChannelItem) cobjs[i];
            userChannelList.add(user);
        }
        Object[] cobjs1 = (Object[]) getIntent().getSerializableExtra("all_data");
        allChannelList = new ArrayList<ChannelItem>(cobjs1.length);
        for (int i = 0; i < cobjs1.length; i++) {
            ChannelItem user = (ChannelItem) cobjs1[i];
            allChannelList.add(user);
        }
