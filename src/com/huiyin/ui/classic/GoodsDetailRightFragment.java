...
public class GoodsDetailRightFragment extends Fragment implements OnClickListener {
	...
	public void onClick(View v) {
		...
		case R.id.sure_shop_tianjia:// 确定添加//或者购买
			...
			userId = AppContext.userId;// = "9";// //用户名id
			...
			if (ProductsDetailActivity.showMenuType == 1) {
				purchase = "2";
			} else if (ProductsDetailActivity.showMenuType == 0) {
				purchase = "1";
			}
			addShoppingCar();
			break;
		...
		try {
			...
			if ("1".equals(result)) {
				...
				if (ProductsDetailActivity.showMenuType == 1) {
					...
					// ((ProductsDetailActivity) getActivity()).finish();
				} else {
					// 加入购物车
					((ProductsDetailActivity) getActivity()).toggle();
	...
	private void checkOutWithoutLogin() {
		...
		if (ProductsDetailActivity.showMenuType == 1) {
			...
			Intent intent = new Intent(getActivity(), WriteOrderActivity.class);
			intent.putExtra(WriteOrderActivity.INTENT_KEY_GOODS_LIST,listItems.toArray());
