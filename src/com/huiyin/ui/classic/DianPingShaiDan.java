...
public class DianPingShaiDan extends FragmentActivity implements OnClickListener {
		...
		if (state == 0) {
			...
			dian_ping_shai_dan_hScrollview.post(new Runnable() {
				...
				public void run() {
					...
					dian_ping_shai_dan_hScrollview.fullScroll(HorizontalScrollView.FOCUS_RIGHT);
				}
			});
		}
	...
	public void dianPingHttp() {
		...
		RequstClient.getDianPingShu(commodity_id, new CustomResponseHandler(this) {
			@Override
			public void onSuccess(int statusCode, Header[] headers, String content) {
				...
				try {
					...
					if ("1".equals(result)) {
						// 请求成功
						dianPingBean = new Gson().fromJson(obj.getString("review"), DianPingBean.class);
						isSuccess = true;
					} else {
						isSuccess = false;
	...
	public void getData(final int pageIndex, String flag) {
		...
		RequstClient.sunSingle(10 + "", pageIndex + "", commodity_id, flag, new CustomResponseHandler(this) {
			@Override
			public void onSuccess(int statusCode, Header[] headers, String content) {
				super.onSuccess(statusCode, headers, content);
				normal();
				try {
					...
					if ("1".equals(result)) {
						// 请求成功
						testBean = new Gson().fromJson(content, DianPingShaiDanList.class);
						...
						if (list != null && list.size() > 0) {
							...
							Toast.makeText(DianPingShaiDan.this, "已无更多数据！", Toast.LENGTH_SHORT).show();
							hide();
						}
					} else {
						Toast.makeText(getApplicationContext(), obj.getString("msg"), Toast.LENGTH_LONG).show();
	...
	private void footer() {
		...
		view = (LinearLayout) LayoutInflater.from(this).inflate(R.layout.xlistview_footer, null);
