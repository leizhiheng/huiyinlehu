...
public class ServiceCardActivity extends BaseActivity implements IXListViewListener {
	...
	protected void onResume() {
		super.onResume();
		String cardNum = AppContext.getInstance().mUserInfo.cardNum;
		cardNum = "卡号：<font color=\"#3592e2\">" + cardNum + "</font>";
		top_cardNum.setText(Html.fromHtml(cardNum));
		
		double balance = AppContext.getInstance().mUserInfo.getBalanceDouble();
	...
	private void laodPageData(int loadType) {
		...
		CustomResponseHandler handler = new CustomResponseHandler(this, true) {
			...
			public void onRefreshData(String content) {
				...
				if (data.type == 1) {
					if (data.balance != 0) {
						AppContext.getInstance().mUserInfo.balance = data.balance+"";
						String priceT = "余额：<font color=\"red\">"
		...
		String token = AppContext.getInstance().mUserInfo.token;
		String cardNum = AppContext.getInstance().mUserInfo.cardNum;
			...
			public void onRefreshData(String content) {
				...
				if (bean.type > 0) {
					Toast.makeText(mContext, "解除绑定成功", Toast.LENGTH_SHORT)
							.show();
					AppContext.getInstance().mUserInfo.setBDStatus(0);
					finish();
				} else {
