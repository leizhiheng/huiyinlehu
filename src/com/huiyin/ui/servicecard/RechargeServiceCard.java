...
public class RechargeServiceCard extends BaseActivity {
	...
	private void payRequst() {
		...
		RequstClient.appAddRechargeOrder(AppContext.getInstance().userId,
				AppContext.getInstance().mUserInfo.cardNum, moneyText,
				AppContext.getInstance().mUserInfo.token, payMode + "", pid
	...
	private void ylPay(String orderId) {
		RequstClient.payForUP(URLs.TN_URL, orderId, new CustomResponseHandler(
				this, true) {
			@Override
			...
			public void onSuccess(int statusCode, String content) {
				...
				UPPayAssistEx.startPayByJAR(RechargeServiceCard.this,
	...
	private void setListener() {
		privilege_chose
				.setOnCheckedChangeListener(new OnCheckedChangeListener() {
                    ...
                    public void onCheckedChanged(RadioGroup group, int checkedId) {
                        if (checkedId == -1)
                            return;
                        RadioButton mRadioButton = (RadioButton) group
                                .findViewById(checkedId);
                        if (mRadioButton.isChecked()) {
                            PrivilegeItem temp = (PrivilegeItem) mRadioButton
                                    .getTag();
                            pid = temp.id;
                            moneytext = MathUtil
                                    .priceForAppWithOutSign(temp.money);
                            money.setText(MathUtil
                                    .priceForAppWithOutSign(temp.money));
		...
		zhifubao.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				if (money.getText().toString().equals("")) {
					Toast.makeText(mContext, "请输入要充值的金额", Toast.LENGTH_LONG)
							.show();
					return;
				}
				payMode = 1;
		...
		payway_wx.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				if (money.getText().toString().equals("")) {
					Toast.makeText(mContext, "请输入要充值的金额", Toast.LENGTH_LONG)
		...
		money.addTextChangedListener(new TextWatcher() {
			...
			public void beforeTextChanged(CharSequence s, int start, int count,
					int after) {
				// Log.d("beforeTextChanged", s + "+" + start + "+" + count +
				// "+" + after);
			}
	...
	private void pay(String orderId) {
		...
		case 2: // 银联
			...
			break;
		case 3: // 微信
            double price= Utils.anDouble(money.getText().toString());
            PayUtil wxapi = new PayUtil(mContext, orderId, mBody, String.valueOf((int) (price * 100)));
            wxapi.Pay();
			break;
		}
	...
	public void startPay(String orderString) {
		...
		alipay.setPayResultCallBack(new PayCallBack() {
			...
			public void payResultCallBack(int type, String resultStatus) {
				if (type == 0) {// 支付成功
					...
					AppContext.getInstance().mUserInfo.balance += Float
							.valueOf(money.getText().toString());
					goToPayResult(1);
				} else {// 支付失败
					goToPayResult(2);
				}
		...
		alipay.alipayOrder(orderString, mTitle, detail,
				String.valueOf(money.getText().toString()));
	}
	...
	public void requestBackToWeb(int type, String tn, final String resultStatus) {
		RequstClient.toRequestWebResult(type, orderId, tn,
				new CustomResponseHandler(this, true) {
					@Override
					public void onFinish() {
						super.onFinish();
	...
	private void UpdataView(ArrayList<PrivilegeItem> data) {
		...
		for (PrivilegeItem temp : data) {
			RadioButton mRadioButton = (RadioButton) LayoutInflater.from(
					mContext).inflate(R.layout.privilege_radiobutton, null);
