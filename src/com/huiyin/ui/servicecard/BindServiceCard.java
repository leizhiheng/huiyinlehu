...
public class BindServiceCard extends BaseActivity{
	...
	private void initData(){
		...
		RequstClient.appBindCardInfo(AppContext.getInstance().userId,cardText,pwdText, new CustomResponseHandler(this) {
			@Override
			public void onSuccess(int statusCode, Header[] headers,
				...
				try {
					...
					if(obj.has("token")){
						AppContext.getInstance().mUserInfo.token = obj.getString("token");
						AppContext.getInstance().mUserInfo.cardNum = card_number.getText().toString();
						AppContext.getInstance().mUserInfo.bdStatus = "1";
						AppContext.getInstance().mUserInfo.balance = obj.optString("balance");
					}
	...
	private void validationCard(){
		String cardText = card_number.getText().toString();
		RequstClient.appValidationCardInfo(cardText,AppContext.getInstance().mUserInfo.token, new CustomResponseHandler(this) {
			@Override
			public void onSuccess(int statusCode, Header[] headers,
					String content) {
				super.onSuccess(statusCode, headers, content);
