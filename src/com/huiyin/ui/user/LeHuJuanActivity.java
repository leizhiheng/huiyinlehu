...
public class LeHuJuanActivity extends BaseActivity {
	...
	private void getLeHuList(final int which_type, int pageIndex, final boolean initB) {
		...
		RequstClient.getLeHuList(AppContext.userId, which_type + "", pageIndex + "", new CustomResponseHandler(this) {
			@Override
			public void onStart() {
				if (initB)
					super.onStart();
	...
	private void addAllTitle() {
		...
		ImageButton upload_voucher = (ImageButton) view.findViewById(R.id.upload_voucher_imageButton);
		ImageButton voucher_rule = (ImageButton) view.findViewById(R.id.voucher_rule_imageButton);
		upload_voucher.setOnClickListener(new OnClickListener() {
			
			@Override
			public void onClick(View v) {
				Intent intent = new Intent(mContext, LehuVoucherActivity.class);
				startActivity(intent);
			}
		});
		voucher_rule.setOnClickListener(new OnClickListener() {
	...
	private class OnClick implements OnClickListener {
		...
		public void onClick(View v) {
			...
			if (StringUtils.isBlank(code)) {
				Toast.makeText(mContext, "请输入乐虎券编号", Toast.LENGTH_SHORT).show();
				return;
			}
			...
			RequstClient.postCodeValidate(AppContext.userId, code, handler);
		}
