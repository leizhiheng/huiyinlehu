...
public class AccountManageActivity extends BaseActivity {
	...
	TextView account_added_invoice,account_address, left_ib, middle_title_tv;
	ImageView account_head;

	//add by zhyao @2015/6/8  添加我的红包
	TextView account_username, account_modify, account_level, account_jifen, account_hongbao;
	TextView account_tie, account_regist_time, account_last_login,account_safety;
	...
	private void initData() {
		...
		// add by zhyao @2015/6/8 设置我的红包金额
		account_hongbao.setText(MathUtil.priceForAppWithSign(mUserInfo.bonus));
	...
	private void initView() {
		...
		// add by zhyao @2015/6/8  添加我的红包入口
		hongbao = (RelativeLayout) findViewById(R.id.myhongbao);
		hongbao.setOnClickListener(new OnClickListener() {

			@Override
			public void onClick(View v) {
				startActivity(new Intent(mContext, MyHongbaoActivity.class));
			}
        ...
        account_added_invoice = (TextView) findViewById(R.id.account_added_invoice);
        account_added_invoice.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View arg0) {
                Intent i = new Intent();
                i.setClass(AccountManageActivity.this, AddedInvoiceActivity.class);
                startActivity(i);
			}
		});
		...
		// add by zhyao @2015/6/8 设置我的红包金额
		account_hongbao = (TextView) findViewById(R.id.account_hongbao);
		...
		account_safety = (TextView) findViewById(R.id.account_security);
		
		account_safety.setOnClickListener(new OnClickListener() {
			
			@Override
			public void onClick(View arg0) {
				LogUtil.d(TAG,"go to AccountLevelActivity");
				startActivity(new Intent(AccountManageActivity.this,AccountSecurityActivity.class));
			}
		});
		...
		account_level.setOnClickListener(new OnClickListener() {
			...
			@Override
			public void onClick(View v) {
				Log.d(TAG,"go to AccountLevelActivity");
				Intent i = new Intent(AccountManageActivity.this, AccountLevelActivity.class);
				startActivity(i);
			}
		});
	}
	...
	private void doModify() {
		...
		RequstClient.doModifyUsername(AppContext.userId, change_name, new CustomResponseHandler(this) {
			@Override
			public void onSuccess(int statusCode, Header[] headers, String content) {
				super.onSuccess(statusCode, headers, content);
	...
	private UpLoadImageListener mUpImageListener = new UpLoadImageListener() {
		...
		public void UpLoadSuccess(ArrayList<String> netimageurls) {
			if (netimageurls == null || netimageurls.size() == 0)
				return;
			if (AppContext.mUserInfo != null) {
				AppContext.mUserInfo.img = netimageurls.get(0);
			}
			DisplayImageOptions options = new DisplayImageOptions.Builder().cacheInMemory(true).cacheOnDisc(true)
					.showStubImage(R.drawable.default_head).showImageForEmptyUri(R.drawable.default_head)
					.showImageOnFail(R.drawable.default_head).imageScaleType(ImageScaleType.IN_SAMPLE_INT)
