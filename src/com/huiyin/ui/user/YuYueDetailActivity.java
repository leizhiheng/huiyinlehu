...
public class YuYueDetailActivity extends BaseActivity {
	...
	/**订单ID**/
	public static final String EXTRA_BESPEAKID 	= "bespeakId";
	
	/**0,返回直接退出    1返回到主界面**/
	public static final String EXTRA_PUSHFLAG 	= "pushFlag";
	...
	TextView yy_type, yy_part, yy_username, yy_phone, yy_addr, yy_remark, yy_audit_remark, yy_audit_remark_tip;
	...
	private void initView() {
		...
		orderId = getStringExtra(EXTRA_BESPEAKID);
		pushFlag = getStringExtra(EXTRA_PUSHFLAG, "0");
		...
		yy_audit_remark_tip = (TextView) findViewById(R.id.yy_audit_remark_tip);

		cancel_btn_tv = (TextView) findViewById(R.id.cancel_btn_tv);
		cancel_btn_tv.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View arg0) {
				Intent i = new Intent(YuYueDetailActivity.this, CommonConfrimCancelDialog.class);
				i.putExtra(CommonConfrimCancelDialog.TASK, CommonConfrimCancelDialog.TASK_CANCEL_YUYUE);
	...
	private void refleshUI() {
		...
		yy_remark.setText(mYuYueInfoBean.bespeakOrderDetail.getRemarks());
		...
		RequstClient.postYuYueInit(orderId, new CustomResponseHandler(this) {
			...
			public void onSuccess(int statusCode, Header[] headers,
				...
				try {
					...
					mYuYueInfoBean = new Gson().fromJson(content, YuYueInfoBean.class);

					refleshUI();
					
					
					//显示管理员审核信息
					String audit_remark = JSONParseUtils.getString(obj.optJSONObject("bespeakOrderDetail"), "AUDIT_REMARKS");
					if(!TextUtils.isEmpty(audit_remark)){
						yy_audit_remark.setText(audit_remark);
						yy_audit_remark.setVisibility(View.VISIBLE);
						yy_audit_remark_tip.setVisibility(View.VISIBLE);
					}else{
						yy_audit_remark.setVisibility(View.GONE);
						yy_audit_remark_tip.setVisibility(View.GONE);
					}
