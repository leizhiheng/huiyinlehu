...
import com.huiyin.bean.AmendReturnInitBean;
 ...
 */
public class ReturnUpdateActivity extends BaseCameraActivity implements OnItemClickListener, OnClickListener {
	...
	/**换货详情数据**/
	private AmendReturnInitBean initBean;
	...
	private void initData(){
		...
		//设置售后类型数据
		lvsaleaftertype.setText("我要退货");
		
		//加载 初始化数据
		mReturnId = getStringExtra(EXTRA_RETURN_ID);
		loadAmendReturnInit();
	}
	 ...
	 * 修改换货初始化
	 */
	private void loadAmendReturnInit(){
		
		RequstClient.amendReturnInit(new CustomResponseHandler(mContext, true) {
			@Override
			public void onRefreshData(String content) {
				
				//解析json
				initBean = AmendReturnInitBean.explainJson(content, mContext);
				if(initBean.type == 1){
	...
	private void showData(){
		...
		if(null == initBean || null == initBean.returnDetailList){
			return;
		}
		
		//显示商品信息
		adapter.refreshData(initBean.returnDetailList);
		...
		//描述问题
		remarkinfo.setText(initBean.DESCRIBE);
	...
	private void amendReplace(String img){
		...
		RequstClient.amendReturn(new CustomResponseHandler(mContext, true) {
			@Override
			public void onRefreshData(String content) {
		...
		}, initBean.ORDER_ID, mReturnId, userId, reasonId, reasonName, goodsList, describe, img, origin);
	}
	...
	public void onClick(View view) {
		...
		case R.id.apply_reason_layout:
			...
			//申请理由
			showReasonPopwindow(view, getDataMap(initBean.getReturnList()), new IOnItemClick() {
				@Override
