...
public class CommitOrderActivity extends BaseActivity implements OnClickListener {
    ...
    // add by zhyao @2015/5/8  添加中行布局
	private LinearLayout payway_china_bank;
    // add by zhyao @2015/5/8  添加建行布局
	...
	private void initView() {
	    ...
	    // add by zhyao @2015/5/8  添加中行布局
		payway_china_bank = (LinearLayout) findViewById(R.id.payway_china_bank);
	    // add by zhyao @2015/5/8  添加银行布局
		payway_construction_bank = (LinearLayout) findViewById(R.id.payway_construction_bank);
	    ...
	    // add by zhyao @2015/5/8  添加中行监听器
		payway_china_bank.setOnClickListener(this);
	    // add by zhyao @2015/5/8  添加建行监听器
		...
		if (AppContext.getInstance().mUserInfo != null && AppContext.getInstance().mUserInfo.getBDStatus() == 1) {
			...
			RequstClient.payForAlipay(URLs.ALIPAY_BEFORE, number, new CustomResponseHandler(this, true) {
				...
				public void onSuccess(int statusCode, String content) {
					...
					try {
						...
						json.optInt("type");
						json.optString("msg");
						if (!TextUtils.isEmpty(json.optString("realMoney"))&&price ==
                                Float.parseFloat(json.optString("realMoney"))) {
...
//                startActivity(mIntent);
			...
			break;
		// add by zhyao @2015/5/8  添加中行
		case R.id.payway_china_bank://中国银行
			payType = 5;
			AppContext.payType = 5;
			Intent intent = new Intent(CommitOrderActivity.this, BankPayActivity.class);
			...
			break;
		// add by zhyao @2015/5/8  添加建行
		case R.id.payway_construction_bank://建行
			payType = 6;
			AppContext.payType = 6;
			Intent intent1 = new Intent(CommitOrderActivity.this, BankPayActivity.class);
